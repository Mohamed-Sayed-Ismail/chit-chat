    ...
    audios.push(audio)
    // Handle both Blob objects and blob URL strings
    audio.src = typeof blob === 'string' ? blob : URL.createObjectURL(blob);
    ...
    (async function () {
        if (typeof blob === 'string') {
            // For blob URLs, wait for audio metadata to load
            await new Promise((resolve) => {
                audio.addEventListener('loadedmetadata', resolve, { once: true });
            });
            duration.value = Math.floor(audio.duration);
        } else {
            // For Blob objects, use getBlobDuration
